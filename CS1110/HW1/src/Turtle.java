...
public class Turtle {
	/// version number based on date of creation
		...
		public Turtle(World w, double x, double y) {
			this.location = new Point2D.Double(x+w.CENTER_X, y+w.CENTER_Y);
		...
		void draw(Graphics2D g) {
			...
			/* old arrow-drawing code

			final double lr = 6;
			final double f = 10;
			final double b = f*0.25;
			...
			arrow.moveTo(location.x, location.y); // center of arrow
			arrow.lineTo(location.x - b*c + lr*s, location.y - b*s - lr*c); // left tail of arrow
			arrow.lineTo(location.x + f*c, location.y + f*s); // tip of arrow
			arrow.lineTo(location.x - b*c - lr*s, location.y - b*s + lr*c); // right tail of arrow
			arrow.closePath();
		...
		public Point2D getLocation() {
			return new Point2D.Double(this.location.x - this.world.CENTER_X, this.location.y - this.world.CENTER_Y);
		}
		...
		public void goTo(Point2D where) {
			this.goTo(where.getX(), where.getY());
		}
		/**
		 * Move the turtle to a particular location. It might leave a trail depending on if the pen is down or not.
		...
		public void goTo(double x, double y) {
			...
			this.location.y = y;
			if (this.isdown) {
				world.drawLine(this.location, ox, oy, this.width, this.color);
				// this.trail.lineTo(this.location.x, this.location.y); // not used
			} else {
				world.turtleMoved();
				// this.trail.moveTo(this.location.x, this.location.y); // not used
			}
